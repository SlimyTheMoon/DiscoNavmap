/* ==========================================================================
   THIRD-PARTY: jQuery Cookie Plugin
   ========================================================================== */
...
/* ==========================================================================
   ...
   ========================================================================== */
// NOTE: dataRootPath is defined in index.html before this script loads
// var dataRootPath = "data/v5.3p2h4/"; 
var currentSystem;
...
var idsNameRegex = /ids_name = ([^;\r\n]*)/g;
var idsSysNameRegex = /strid_name = ([^;\r\n]*)/g;
var atmosphereRadiusRegex = /atmosphere_range = ([^;\r\n]*)/g;
var sysNameRegex = /system = ([^;\r\n]*)/g;
var idsInfoRegex = /ids_info = ([^;\r\n]*)/g;
var scaleRegex = /navmapscale = ([^;\r\n]*)/g;
var commentRegex = /[\n\r]+;+.*/g;
var objectRegex = /(\n[^\r\n;]*\[[Oo]bject\])([^\[]*(?=\n\w*|$))/g;
var zoneRegex = /(\n[^\r\n;]*\[[Zz]one\])([\s\S]*?)(?=\n\[|$)/g;
var zoneNicknameRegex = /zone = ([^\r\n;]*)/g;
var asteroidsRegex = /(\[[Aa]steroids\])(\r\n.+)*/g;
var lootableZoneRegex = /(\n[^\r\n;]*\[[Ll]ootable[Zz]one\])([\s\S]*?)(?=\n\[|$)/g;
var groupRegex = /(\n[^\r\n;]*\[[Gg]roup\])([\s\S]*?)(?=\n\[|$)/g;
var commodityRegex = /(\n[^\r\n;]*\[[Cc]ommodity\])([\s\S]*?)(?=\n\[|$)/g;
var ambientRegex = /(\n[^\r\n;]*\[[Aa]mbient\])([\s\S]*?)(?=\n\[|$)/g;
// Improved regex to handle start of file better
var systemRegex = /(\n[^\r\n;]*\[[Ss]ystem\])([\s\S]*?)(?=\n\[|$)/g;
var solarRegex = /(\n[^\r\n;]*\[[Ss]olar\])([\s\S]*?)(?=\n\[|$)/g;
var baseRegex = /(\n[^\r\n;]*\[[Bb]ase\])([\s\S]*?)(?=\n\[|$)/g;
var posRegex = /pos = (.*)/g;
...
var archetypeIgnoreArray = [
    ...
    "suprise_b_battleship_secret", "invisible_base", "blhazard", "li17_suprise_bw_elite2_01", "suprise_hf_elite2_concealed"
];
...
// -- Out of Roleplay (OORP) Systems --
var oorpArray = [
    "br09", "br10", "bw11", "bw14", "ca01", "ev01", "ev02", "ev03", "ew05",
    "ew14", "ew19", "ew20", "ew21", "ew37", "ew63", "ew85", "fp7_system",
    "hi19", "hlp1", "hlp2", "iw09", "ku17", "li06", "li07", "li08", "limbo",
    "st02c", "st03b", "unch01", "unch02", "unch03", "unch04", "unch04b",
    "unch05", "unch06", "unch07", "unch08", "unch09", "unch10", "vr01"
];

// -- State & Data Arrays --
var searchTimedOut = "nope";
var universeFileGetResult = ""; // Initialized to string to prevent null errors
...
/* ==========================================================================
   INITIALIZATION
   ========================================================================== */
// NOTE: Initialization is triggered from index.html which calls:
// - generateLookupArrays()
// - generateUniverseMap()  
// - startSearchField()
// - checkURL()
// The $(document).ready() below only sets up event handlers and restores settings
...
var URIHash = {
    dump: function() {
        ...
        for (var key in hash) {
            ...
            if (pair.length != 2 || pair[0] in dump) return undefined;
    ...
    set: function(key, value) {
        ...
        dump[key] = value;
        var hash = new Array();
        for (var key in dump) hash.push(escape(key) + '=' + escape(dump[key]));
...
function updateConfigClasses() {
    ...
    if (document.querySelector(".configOption#connections input").checked && document.querySelector(".systemConnectionProp") == null) {
        ...
        $(".systemConnectionProp").hide();
    } else if (document.querySelector(".configOption#connections input").checked && document.querySelector(".contents .system") !== null) {
    ...
    if (document.querySelector(".configOption#universeLabels input").checked && document.querySelector(".contents .system") !== null) {
    ...
    if (document.querySelector(".configOption#systemLabels input").checked && document.querySelector(".contents .system") == null) {
        $(".contents div label").removeClass("labelDisabled");
        $(".contents div label").hAlign();
    } else if (document.querySelector(".contents .system") == null) {
        $(".contents div label").addClass("labelDisabled");
    }
    if (document.querySelector(".configOption#showAllObjects input").checked && document.querySelector(".contents .system") == null) {
        $(".contents").addClass("showAllObjects");
        $(".contents div label").hAlign();
    } else if (document.querySelector(".contents .system") == null) {
        $(".contents").removeClass("showAllObjects");
    }
    if (document.querySelector(".configOption#showAllObjectLabels input").checked && document.querySelector(".contents .system") == null) {
        $(".contents").addClass("showAllObjectLabels");
        $(".contents div label").hAlign();
    } else if (document.querySelector(".contents .system") == null) {
        $(".contents").removeClass("showAllObjectLabels");
    }
    if (document.querySelector(".configOption#showInfocardedObjectLabels input").checked && document.querySelector(".contents .system") == null) {
...
function generateLookupArrays() {
    ...
    $.get(systemNameFile, function(data) {
        ...
        alert("Error: Could not load " + systemNameFile);
    });
    ...
    $.get(commodityFile, function(data) {
        ...
        console.warn("Could not load " + commodityFile);
    });
    ...
    $.get(initialWorldFile, function(data) {
        ...
        if (groupArray) {
            console.log("DEBUG: Found " + groupArray.length + " groups, first group sample:", groupArray[0].substring(0, 200));
            for (i = 0; i < groupArray.length; i++) {
                if (groupArray[i].toLowerCase().indexOf("nickname =") != -1 && groupArray[i].toLowerCase().indexOf("ids_name =") != -1) {
                    var idsNameString = groupArray[i].toLowerCase().match(idsNameRegex).join().substring(11).trim();
                    var nameString = groupArray[i].toLowerCase().match(nameRegex).join().substring(11).trim();
                    factionNameArray[nameString.toLowerCase().trim()] = idsNameString;
                }
            }
            console.log("DEBUG: factionNameArray has " + Object.keys(factionNameArray).length + " entries. Sample keys:", Object.keys(factionNameArray).slice(0, 5));
        }
    ...
    $.get(infocardFile, function(data) {
        ...
        alert("CRITICAL ERROR: " + infocardFile + " not found. The map cannot name anything.");
    });
    ...
    $.get(universeFile, function(data) {
        ...
        alert("CRITICAL ERROR: " + universeFile + " not found. The map cannot locate systems.");
    });
    generateSearchArray();
}
...
function showObjectInfo(idsNameNumber, idsInfoNumber, classString, zPosition, factionNickname, systemNickname, dynamicCommodity, dynamicDifficulty, dynamicCount, nickname) {
    ...
    if (clickHandlersEnabled) {
        ...
        console.log("DEBUG ownerString: factionNickname=", factionNickname, "normFaction=", normFaction, "factionNameArray[normFaction]=", factionNameArray[normFaction], "infocardArray lookup=", infocardArray[factionNameArray[normFaction]]);
        ...
        if (typeof idsInfoNumber !== "undefined") {
            if (typeof infocardMapArray[(parseInt(idsInfoNumber)).toString()] !== "undefined" && typeof infocardArray[infocardMapArray[(parseInt(idsInfoNumber)).toString()]] !== "undefined") {
                ...
                contentHTML = "<h2>" + infocardArray[idsNameNumber] + "</h2>" +
                    parseInfocard(infocardArray[idsInfoNumber]) +
                    parseInfocard(infocardArray[infocardMapArray[(parseInt(idsInfoNumber)).toString()]]) +
                    "<h3>Technical info</h3>" + miningString +
                    "<p class='technicalInfo'>This object with internal nickname " + nickname + " is located " + planePosition + " the plane, and has name infocard number " + idsNameNumber + ", infocard number " + idsInfoNumber + " and base description infocard number " + (parseInt(idsInfoNumber) + 1).toString() + "." + ownerString + " " + "<a class='editLink' target='_blank'><i class='fa fa-pencil-square-o'></i></a></p>" + scrollUpString;
                setTimeout(function() {
                ...
                contentHTML = "<h2>" + infocardArray[idsNameNumber] + "</h2>" +
                    parseInfocard(infocardArray[idsInfoNumber]) +
                    "<h3>Technical info</h3>" + miningString +
                    "<p class='technicalInfo'>This object with internal nickname " + nickname + " is located " + planePosition + " the plane, and has name infocard number " + idsNameNumber + " and infocard number " + idsInfoNumber + "." + ownerString + " " + "<a class='editLink' target='_blank'><i class='fa fa-pencil-square-o'></i></a></p>" + scrollUpString;
                setTimeout(function() {
            ...
            contentHTML = miningString + scrollUpString;
        }
        document.querySelector(".infocardContainer").innerHTML = contentHTML;
...
var universeMapRetries = 0;
function generateUniverseMap() {
    // Check if dependencies are ready
    var systemConnectionsEvaluated = true;
    if (document.querySelector(".configOption#connections input") && document.querySelector(".configOption#connections input").checked) {
        if (systemConnectionState != "ready") {
            systemConnectionsEvaluated = false;
        }
    }
    ...
    if (infocardArrayState != "ready" || systemScaleFactorArrayState != "ready" || searchArrayState != "ready" || systemConnectionsEvaluated != true || systemNameArrayState != "ready") {
        ...
        // Show loading screen on first wait
        if (universeMapRetries == 1) {
             createLoadAnimation();
        }

        // 60 Second Timeout (600 iterations * 100ms)
        if (universeMapRetries > 600) {
             console.warn("Map generation is taking a long time. Check console for missing file errors (404).");
        }
        ...
        universeMapRetries = 0;
        createLoadAnimation(); // Ensure animation is present during build
        ...
        document.querySelector(".grid").style.background = "url('./images/Sirius_Map.png') black";
            ...
            for (var system in connectionArray) {
                if (connectionArray.hasOwnProperty(system)) {
                    for (i = 0; i < connectionArray[system].length; i++) {
                        if (typeof systemPosArray[system] !== "undefined" && typeof systemPosArray[connectionArray[system][i]] !== "undefined") {
            ...
            for (var system in jgConnectionArray) {
                if (jgConnectionArray.hasOwnProperty(system)) {
                    for (i = 0; i < jgConnectionArray[system].length; i++) {
                        if (typeof systemPosArray[system] !== "undefined" && typeof systemPosArray[jgConnectionArray[system][i]] !== "undefined") {
...
var mapGenerationRetries = 0;
function generateMap(system) {
    if (infocardArrayState != "ready" ||
        systemScaleFactorArrayState != "ready" ||
        searchArrayState != "ready" ||
        document.querySelector(".loadingOverlay") !== null) {

        mapGenerationRetries++;
        
        // 60 Second timeout
        if (mapGenerationRetries > 600) {
            console.warn("Map Generation taking a long time...");
        }
        ...
        if (system.toLowerCase() == "fp7_system") {
            systemFile = dataRootPath + "universe/systems/fp7/fp7_system.ini";
        } else {
            systemFile = dataRootPath + "universe/systems/" + system + "/" + system + ".ini";
        }
        ...
        $.get(systemFile, function(unsanitizedData) {
                        ...
                        $.get(asteroidsURIArray[asteroidsNickname], function(unsanitizedData) {
                            ...
                            for (zoneNumber in lootableZones) {
                                ...
                                if (data.indexOf("[LootableZone]") != -1 &&
                                    ((data.indexOf("dynamic_loot_commodity = ") != -1 || data.indexOf("asteroid_loot_commodity = ") != -1)) &&
                                    data.match(lootableZoneRegex)) {
            ...
            if (sysZoneArray !== null) {
                for (i = 0; i < sysZoneArray.length; i++) {
                    try {
                        var nameMatch = sysZoneArray[i].match(nameRegex);
                        if (!nameMatch) continue;
                        var nameString = nameMatch.join().substring(11).trim();
                        var posMatch = sysZoneArray[i].match(posRegex);
                        if (!posMatch) continue; // Skip zones without position
                        if (asteroidsURIArray[nameString.toLowerCase()] != null || sysZoneArray[i].indexOf("66170") == -1) {
                            ...
                            if (sysZoneArray[i].indexOf("ids_name") != -1) {
                                var idsNameMatch = sysZoneArray[i].match(idsNameRegex);
                                if (idsNameMatch) {
                                    var idsNameNumber = idsNameMatch.join().substring(11).trim();
                            ...
                            var posString = posMatch.join().substring(6).replace(/ /g, "");
                            ...
                            if (sysZoneArray[i].indexOf("ids_info") != -1 && sysZoneArray[i].match(idsInfoRegex)) {
                                var idsInfoMatch = sysZoneArray[i].match(idsInfoRegex);
                                var idsInfoNumber = idsInfoMatch ? idsInfoMatch.join().substring(11).trim() : undefined;
...
function mapObject(objectClasses, posX, posY, posZ, rotX, rotY, rotZ, label) {
    ...
    if (!!document.querySelector(".playerShip.previousLatestPos")) {
        ...
        var xDiff = parseFloat(lastPos.style.top.slice(0, -1)) * 2000 / systemScaleFactor - parseFloat(posX);
        var zDiff = parseFloat(lastPos.style.left.slice(0, -1)) * 2000 / systemScaleFactor - parseFloat(posZ);
...
/* ==========================================================================
   COLLISION DETECTION (LABEL OVERLAP)
   ========================================================================== */
...
function moveIfOverlapsAndReturnDiff(currentObject, objectArray) {
    var diffSum = 0;
    var reducedObjectArray = objectArray;
